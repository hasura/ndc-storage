...
func (c *Client) UpdateObject(ctx context.Context, bucketName string, objectName string, opts common.UpdateStorageObjectOptions) error {
	...
	if opts.VersionID != "" {
		...
		if err != nil {
			return schema.UnprocessableContentError(fmt.Sprintf("invalid generation version: %s", err), nil)
		}
...
func (c *Client) presignObject(ctx context.Context, method string, bucketName string, objectName string, opts common.PresignedGetStorageObjectOptions) (string, error) {
