...
func (m *Manager) ListIncompleteUploads(
	ctx context.Context,
	bucketInfo common.StorageBucketArguments,
	opts common.ListIncompleteUploadsOptions,
...
func (m *Manager) GetObject(
	...
	if objectStat.IsDirectory {
		return nil, nil, schema.UnprocessableContentError(
			"cannot download directory: "+objectName,
			nil,
		)
	}
	...
	if objectStat.Size == nil || *objectStat.Size > m.runtime.MaxDownloadSizeMBs*1024*1024 {
		return nil, nil, schema.UnprocessableContentError(
			fmt.Sprintf(
				"file size > %d MB is not allowed to be downloaded directly. Please use presignedGetObject function for large files",
				m.runtime.MaxDownloadSizeMBs,
			),
			nil,
		)
	}
...
func (m *Manager) RemoveIncompleteUpload(
	ctx context.Context,
...
func (m *Manager) PresignedGetObject(
	ctx context.Context,
	bucketInfo common.StorageBucketArguments,
	objectName string,
	...
	if exp == 0 {
		return nil, schema.UnprocessableContentError(
			"expiry is required and must be larger than 0",
			nil,
		)
	}
...
func (m *Manager) PresignedPutObject(
	ctx context.Context,
	bucketInfo common.StorageBucketArguments,
	objectName string,
...
func (m *Manager) UploadObjectFromURL(
	ctx context.Context,
	bucketInfo common.StorageBucketArguments,
	objectName string,
	httpRequest *common.HTTPRequestOptions,
	opts *common.PutStorageObjectOptions,
) (*common.StorageUploadInfo, error) {
	client, bucketName, err := m.GetClientAndBucket(ctx, bucketInfo)
	if err != nil {
	...
	if httpRequest.Method == nil ||
		(*httpRequest.Method == "" || *httpRequest.Method == http.MethodGet) {
	...
	if contentType := resp.Header.Get(common.HeaderContentType); contentType != "" &&
		opts.ContentType == "" {
		mediaType, _, err := mime.ParseMediaType(contentType)
		if err != nil {
			return nil, schema.UnprocessableContentError(
				"invalid response content type: "+err.Error(),
				nil,
			)
		}
	...
	if contentLanguage := resp.Header.Get(common.HeaderContentLanguage); contentLanguage != "" &&
...
func (m *Manager) contentLengthFromHEAD(
	ctx context.Context,
	httpRequest *common.HTTPRequestOptions,
) int64 {
	...
	if err != nil {
		slog.Debug(
			fmt.Sprintf("failed to send HEAD request: %s", err),
			slog.String("url", httpRequest.URL),
		)
...
func maxUploadSizeLimitError(mbs int64) error {
	return schema.UnprocessableContentError(
		fmt.Sprintf(
			"file size > %d MB is not allowed to be upload directly. Please use presignedPutObject function for large files",
			mbs,
		),
		nil,
	)
